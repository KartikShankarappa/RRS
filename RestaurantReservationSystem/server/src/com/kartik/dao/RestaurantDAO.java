...
public class RestaurantDAO {
	...
	public Restaurant findRestaurantDetails() throws AppException {
		...
		try {
			ps = con.prepareStatement("select * from restaurant where restaurantId = 1");
			rs = ps.executeQuery();
			...
			if(rs.next()) {
				...
				restaurant.setPhoneNumber(rs.getString("phoneNo"));
	...
	public Restaurant updateRestaurantDetails(Restaurant restaurant) throws AppException {
		...
		try {
			ps = con.prepareStatement("UPDATE restaurant set restaurantName = ?, emailId = ?, address = ?,  city = ?, state = ?, zip = ?, phoneNo = ?, daysOfOperation = ?, openingTime = ?, closingTime = ? where restaurantId = 1");
			...
			ps.setString(8, restaurant.getDaysOfOperation());
