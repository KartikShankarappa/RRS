...
public class ReservationController {
	...
	public Reservation findOneReservation(@PathParam ("id") int confNo) {
		...
		try {
			...
			if (reservationDAO.listOneReservation(confNo) == null) {
	...
	@ApiOperation (value = "Update Reservation",
					notes = "Update aa reservation in the system"
			)
	@ApiResponses (value = {
			@ApiResponse (code = 200, message = "Success"),
			@ApiResponse (code = 400, message = "Bad Request"),
			@ApiResponse (code = 401, message = "Unauthoried Access"),
			@ApiResponse (code = 404, message = "Not Found"),
			@ApiResponse (code = 500, message = "Internal Server Error")
	})	
	public Reservation updateReservation(Reservation reservation) {
		...
		try {
			...
			if(!tableDAO.ifCurrentTableStillOk(reservation)) {
